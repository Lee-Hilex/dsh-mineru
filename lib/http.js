 ...
 * @param {string} deps.settingsNs the profile entry id the settings card is keyed by
...
async function handle(req, res, deps) {
  ...
  // half resolves and passes down; 'mineru' is only the fallback.
  const settingsNs = typeof deps.settingsNs === 'string' && deps.settingsNs.length > 0 ? deps.settingsNs : 'mineru';
  const descriptorOf = () => (deps.settings.describe({ redactSecrets: true }) ?? []).find((d) => d.ns === settingsNs);
  ...
  if (route === '/config' && method === 'GET') {
    ...
    if (descriptor === undefined) {
      ...
      // not active, or when no field carries `meta.volatile`.
      sendJson(res, 503, {
        ok: false,
        namespace: settingsNs,
        error: 'settings namespace "' + settingsNs + '" is not registered',
        hint: 'The host derives this card from the plugin\'s exported Config schema: the entry must be active and declare at least one `meta.volatile` field. '
          + 'Restart DSH after upgrading the plugin, and check that the profile entry id matches the namespace the card asks for.',
      });
      return;
    }
    const facts = await deps.state.collectFacts();
  ...
  if (route === '/config' && method === 'POST') {
    ...
    try {
      await deps.settings.update(settingsNs, patch, Number.isInteger(body.expectedRevision) ? body.expectedRevision : undefined);
      const descriptor = descriptorOf();
...
async function handleUpload(req, res, deps, url) {
  ...
  // Timestamp + random suffix: concurrent/same-second uploads of the same name
  // each land in their own directory instead of colliding on 'wx'.
  const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19) + '-' + randomBytes(3).toString('hex');
