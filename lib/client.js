	...
	factory: (require) => {
		...
		function ArtifactChip(props) {
			...
			return jsxs("span", { style: { display: "inline-flex", alignItems: "center", gap: 6, background: "var(--dsw-alias-bg-module-platform, #20242c)", border: "1px solid var(--dsw-alias-border-l2, #2a2d34)", borderRadius: 8, padding: "3px 10px", fontSize: 12 }, children: [
				...
				a.url ? jsx("a", { href: a.url, target: "_blank", rel: "noopener noreferrer", style: { fontSize: 12, color: "var(--dsw-alias-brand-primary-new-colorprimary-new-color, #4d6bfe)" }, children: "预览" }) : null,
			] }, a.path);
		}
		...
		function installFileDropBridge(ctx) {
			...
			async function intake(files) {
				...
				try {
					...
					toast("已上传 " + uploaded.length + " 个文件并填入输入框: 请补充需求后点击发送.");
				} catch (err) {
					toast("文件已上传, 但写入输入框失败: " + (err?.message ?? String(err)) + " 文件路径: " + uploaded.join("; "), "error");
				}
			}
		...
		// Plugin entry
		// ------------------------------------------------------------------
		var inject = ["slots", "connection", "sessions", "conversation"];
