...
async function runSingleParse({ source, args, cfg, exec, state }) {
  ...
  if (urlSource) {
    ...
    fileName = basename(parsed.pathname) || 'document';
    ext = extOf(fileName);
    if (!SUPPORTED_EXTENSIONS.includes(ext)) fileName = undefined; // let the server parse from the URL
  } else {
...
async function runBatchParse({ sources, args, cfg, exec, state }) {
  ...
  try {
    if (urls.length > 0) {
      const r = await client.submitAndWaitBatchUrls({ items: urls, opts, signal: exec.signal });
      batchIds.push(r.batchId);
      for (const item of r.results) {
        if (item.state === 'done' && item.fullZipUrl) {
          const itemDir = joinSafe(dir, sanitizeFileName(item.name ?? ('item-' + results.length)));
          const collected = await client.collectPrecisionZip({ zipUrl: item.fullZipUrl, destDir: itemDir, opts, signal: exec.signal });
          doneCount += 1;
          const relItemDir = relDir.replace(/\\/g, '/') + '/' + sanitizeFileName(item.name ?? ('item-' + results.length));
...
const BATCH_RESULT_SCHEMA = {
  ...
  properties: {
    ...
    results: { type: 'array', items: { type: 'object', additionalProperties: false, properties: {
      name: { type: 'string' }, state: { type: 'string' }, dir: { type: 'string' },
      errMsg: { type: 'string' }, markdownBytes: { type: 'number' },
      artifacts: { type: 'array', items: ARTIFACT_ITEM_SCHEMA },
    } } },
    artifacts: { type: 'array', items: ARTIFACT_ITEM_SCHEMA },
  },
...
export function buildBatchTool(state) {
  return defineTool({
    ...
    description: '用 MinerU 精准解析 API 批量解析文档 (需要 Token): sources 为工作区文件路径与 http(s) URL 的混合列表, 本地文件单批 <=50 个, URL 单批 <=200 个, 自动分组提交. 每个文档的结果解包到独立子目录 (<run>/<文件名>/full.md). 解析成功与失败逐项列出.',
    parameters: {
    ...
    presentCall(args) {
      ...
      return {
        ...
        kind: 'fetch',
        rawInput: { count: sources.length, modelVersion: args.modelVersion, language: args.language },
      };
    },
    presentResult(args, result) {
  ...
  properties: {
    ...
    progress: { type: 'object', additionalProperties: false, properties: {
      extracted_pages: { type: 'number' }, total_pages: { type: 'number' }, start_time: { type: 'string' },
    } },
    runDir: { type: 'string' },
    preview: { type: 'object', additionalProperties: false, properties: { markdown: { type: 'string' }, truncated: { type: 'boolean' }, bytes: { type: 'number' } } },
    artifacts: { type: 'array', items: ARTIFACT_ITEM_SCHEMA },
